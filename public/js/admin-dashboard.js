...
(function () {
  ...
  async function loadOrders() {
    ...
    setDashboardQuery({
      ...
      o_status: q.status,
      o_tariff: q.tariff,
      o_bracelet: q.bracelet,
      o_date_from: q.dateFrom,
      o_date_to: q.dateTo,
      o_page: q.page,
    });
    const filterQs = Q({ q: q.q, status: q.status, tariff: q.tariff, bracelet: q.bracelet, dateFrom: q.dateFrom, dateTo: q.dateTo });
      ...
      ? rows
        .map((x) => {
          ...
          const menu = menuWrap([
            ...
            menuItem({ label: "��������� ���������", icon: "xCircle", attrs: `data-act="uvd" data-id="${X(x.telegramId)}" data-name="${X(x.name)}" data-slugs="${X(userSlugsCsv)}"`, danger: true }),
            menuSeparator(),
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (a === "uvd") {
      const userId = n.getAttribute("data-id");
      const userName = n.getAttribute("data-name") || "������������";
      if (!userId) return;
      const userSlugs = String(n.getAttribute("data-slugs") || "")
        .split(",")
        .map((slug) => normalizeShortSlug(slug))
        .filter((slug) => isShortSlug(slug));
      if (!userSlugs.length) {
        await showAlert("� ������������ ��� slug ��� �������� ����������.");
        return;
      }
      const enteredCount = await showPrompt(`������� ���������� ������ ��� ${userName}? (1-5000)`, "100");
      if (enteredCount === null) return;
      const count = Number.parseInt(String(enteredCount || "").trim(), 10);
      if (!Number.isFinite(count) || count < 1 || count > 5000) {
        await showAlert("���������� ������ ���� ������ �� 1 �� 5000.");
        return;
      }
      let targetSlug = userSlugs[0];
      if (userSlugs.length > 1) {
        const enteredSlug = await showPrompt(`� ������ slug ������� ���������? (${userSlugs.join(", ")})`, targetSlug);
        if (enteredSlug === null) return;
        targetSlug = normalizeShortSlug(enteredSlug);
        if (!isShortSlug(targetSlug)) {
          await showAlert("Slug ������ ���� � ������� AAA000.");
          return;
        }
      }
      const ok = await showConfirm(`��������� �� ${count} ���������� ��� ${targetSlug} ������������ ${userName}?`);
      if (!ok) return;
      ...
      const payload = await r.json().catch(() => ({}));
      await showAlert(`������� ����������: ${Number(payload?.removedViews || 0)} (slug: ${payload?.slug || targetSlug}).`);
      void loadUsers();
      closeAllRowMenus();
      return;
    }
  ...
  document.getElementById("orders-filters")?.addEventListener("reset", () => {
    ...
    setTimeout(() => {
      ...
      setFormValue(form, "bracelet", "all");
