      ...
      BEGIN
        IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'CardTheme') THEN
          IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'CardTheme_new') THEN
            EXECUTE 'CREATE TYPE "CardTheme_new" AS ENUM (''default_dark'', ''arctic'', ''linen'', ''marble'', ''forest'', ''sage_luxe'', ''midnight_obsidian'', ''golden_noir'')';
          END IF;
          ...
          IF EXISTS (
            ...
            EXECUTE 'ALTER TABLE public.cards ALTER COLUMN theme TYPE "CardTheme_new" USING (
              CASE
                ...
                WHEN theme::text IN (''default_dark'', ''arctic'', ''linen'', ''marble'', ''forest'', ''sage_luxe'', ''midnight_obsidian'', ''golden_noir'')
